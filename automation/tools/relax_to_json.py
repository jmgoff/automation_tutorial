...
def convert_stress_tensor_to_force(v1, v2, v3, stress_tensor):
    # Calculate the surface area for each face of the parallelepiped
    surface_areas = np.array([
        np.linalg.norm(np.cross(v1, v2)),
        np.linalg.norm(np.cross(v2, v3)),
        np.linalg.norm(np.cross(v3, v1))
    ])

    if type(stress_tensor) == list:
        stress_tensor = np.array(stress_tensor)

    # Calculate the force per atom NOTE for single atom unit cells only!
    force_per_atom = stress_tensor * surface_areas[:, None]
    force_per_atom = np.sum(stress_tensor * surface_areas[:, None],axis=1)
    atoms_forces = [list(force_per_atom)]
    return atoms_forces
...
for traj_ind, energy in enumerate(energy_traj):
    ...
    if len(atoms) == 1:
        force_calc_stresses = np.array(stresses)*rypau3_to_gpa*(1/ev_2_gpa)
        force_calc_stresses = force_calc_stresses.tolist()
        forces = convert_stress_tensor_to_force(cell[0],cell[1],cell[2],force_calc_stresses)
    else:
