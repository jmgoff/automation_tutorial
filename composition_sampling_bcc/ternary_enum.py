...
#['Si', 'V']
#]
base_species_pairs = [list(p) for p in itertools.combinations(['Cr','Fe','Si','V'],3)]
cellmaxmult = 4
for base_species in base_species_pairs:
    if not os.path.isdir('%s%s%s_ternary_DSS_%d' % (tuple(base_species) + tuple([cellmaxmult]) ) ):
        os.mkdir('%s%s%s_ternary_DSS_%d' % (tuple(base_species) + tuple([cellmaxmult]) ))
    os.chdir('%s%s%s_ternary_DSS_%d' % (tuple(base_species) + tuple([cellmaxmult]) ))
    ...
    se.compress_expand(axes = [0,1,2],stepsize=0.03, nsteps = 0 )
    os.chdir('..')
